...
class MPPEM(object):
    ...
    Marked Point Process Learning via EM algorithm
    seq_t = time matrix
    seq_s = lat long locations
    seq_m = mark (type of crime, some category)
    '''

    def __init__(self, seq_t, seq_s, seq_m=None, beta=1.):
        # training data
        self.t      = seq_t # time of each of events
        self.s      = seq_s # component of each of events
        ...
        self.d      = len(self.s)          # number of components if using discrete
    ...
    def _loglik_subterm_1(self, i, t_indices):
        ...
        terms = [
            self.A[self.s[i]][self.s[j]] * 1. * np.exp(-1 * self.beta * (self.t[i] - self.t[j]) + np.inner(self.m[i], self.m[j]))
            for j in t_indices[t_indices<i] ]
    ...
    def _loglik_subterm_2(self, uj, t_indices, T):
        ...
        terms = [
            self.A[self.s[i]][uj] * (1 - np.exp(- self.beta * (T - self.t[i])) + np.inner(self.m[i], self.m[t_indices[-1]]))
            for i in t_indices]
        return np.array(terms)

    # TODO use this for likihood from tau to T
    def log_likelihood(self, T, tau):
        ...
        term_1 = [ np.log(self.Mu[self.s[i]] + self._loglik_subterm_1(i, t_indices).sum()) for i in t_indices.tolist() ]
