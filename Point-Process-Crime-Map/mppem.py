...
class MPPEM(object):
    ...
    Marked Point Process Learning via EM algorithm
    seq_t = time matrix
    seq_u = discretized location (beats), components
    seq_l = lat long locations
    ...
    def log_likelihood(self, T, tau):
        ...
        print('t indies:: ', t_indices)

        term_1 = []

        for i in t_indices:
            t_1 = self.Mu[self.u[i]]
            t_2 = self._loglik_subterm_1(i, t_indices)
            final_t = np.log( t_1 + t_2.sum())
            term_1.append(final_t)
